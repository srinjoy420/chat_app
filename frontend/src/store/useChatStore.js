...
export const useChatStore = create((get, set) => ({
    ...
    // Get chat partners (users with whom the current user has chatted)
    getMychatPartners: async () => {
        ...
        try {
            ...
            set({ 
                ...
                error: error.response?.data?.message || "Failed to fetch chat partners", 
                isUsersLoading: false ,
    ...
    sendMessage: async (receiverId, messageData) => {
        ...
        try {
            const res = await axiosInstance.post(`/api/v1/message/send/${receiverId}`, messageData);
            if (res.data.success) {
